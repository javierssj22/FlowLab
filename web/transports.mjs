...
export class WebSerialTransport {
  ...
  async connect(_port,board){
    ...
    try{await new Promise(r=>setTimeout(r,1800));this.info=await this.command('hello');if(![1,2].includes(this.info?.protocol)||this.info.family!==board)throw Error('Firmware o familia incompatible.');return this.status();}catch(e){await this.disconnect();throw e;}
  }
  ...
  command(command,args={}){
    ...
    if(['burst','i2cxfer'].includes(command)&&this.info?.protocol!==2)return Promise.reject(Error('Carga firmware de protocolo 2.'));
