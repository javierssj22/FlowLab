...
export function generateArduino(project) {
  ...
  if(collect(project).some(n=>['vector','waveform'].includes(describe(n).output)))throw Error('Vector, waveform, ADC DMA e I²C multibyte requieren el motor conectado en 0.2; la exportación autónoma admite los bloques escalares y subVIs escalares.');
  ...
  for(const n of executionOrder(project)) {
    ...
    if(['chart','gauge','led','log'].includes(n.type))body.push(`  Serial.print(t,6); Serial.print(",${n.id},"); Serial.println((double)${name},6);`);
